	...
	nh_(nh)
{
	...
	/*if (nh.getParam("rosaria_motion/robot_name", robot_name_))
	{
		ROS_INFO("Starting Rosaria Motion with robot whose name is %s.", robot_name_.c_str());
	}*/
	...
	/** 
	 * criação de um novo publicador do tópico '/beacon', o qual
	 * transmitirá mensagens do tipo 'login_manager/Beacon'
	 */
	beacon_pub_ = nh_.advertise<login_manager::Beacon>("/beacon", 1);

	/** 
	 * criação de um novo assinante do tópico 'pose', o qual 
	 * receberá mensagens do tipo 'nav_msgs/Odometry'
         */
	pose_sub_ = nh_.subscribe("odom", 1, &rosaria_motion::RosariaMotionNode::poseCallback, this);

	/**
	 * criação de um novo servidor de serviço do tipo 'rosaria_motion/SetVelocity'
	 */
	set_velocity_srv_ = nh_.advertiseService("set_velocity", &RosariaMotionNode::setVelocity, this);
}
 ...
 *
 */
void rosaria_motion::RosariaMotionNode::publishBeaconSignal()
{
	login_manager::Beacon beacon_msg;
	beacon_msg.time_stamp = ros::Time::now();
	beacon_msg.name = robot_name_;
	beacon_msg.available = available_;	
	beacon_msg.x = curr_x_;
	beacon_msg.y = curr_y_;
	beacon_msg.theta = curr_phi_;
	beacon_pub_.publish(beacon_msg);
}
